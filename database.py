...
class DatabaseManager ():
    ...
    def get_item(self,item_id):
        self.cursor.execute('''SELECT i.id, i.name, i.description, i.price, c.name, i.category_id
                             FROM items i
    ...
    def update_order(self,full_name,phone_number,address,payment_type,city,order_id):
        self.cursor.execute('''UPDATE orders
                            SET full_name = ?,phone_number = ?,address = ?,payment_type = ? ,city = ?
                            WHERE id  = ?
                             ''',[full_name,phone_number,address,payment_type,city,order_id])
        self.connection.commit()
